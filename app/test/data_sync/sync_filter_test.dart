...
void main() {
  group('SyncFilter.matches', () {
    ...
    test('min/max amount compare on absolute value', () {
      ...
      expect(filter.matches({'amount': -500}), isTrue,
          reason: 'debits are negative');
      expect(filter.matches({'amount': -499.99}), isFalse);
      expect(filter.matches({'amount': '600'}), isTrue,
          reason: 'string coercion');
    ...
    test('accountKeys match account rows but exclude ownerless transactions',
        () {
      const filter = SyncFilter(accountKeys: ['1000001234|1']);
      expect(
          filter.matches({'accountNumber': '1000001234', 'bank': 1}), isTrue);
      expect(filter.matches({'accountNumber': '1234', 'bankId': 1}), isFalse);
      expect(
        filter.matches({
          'accountNumber': '1234',
          'ownerAccountNumber': '1000001234',
          'bankId': 1,
        }),
        isTrue,
      );
    ...
    test('authoritative ownerAccountNumber wins over parsed SMS evidence', () {
      const firstAccount = SyncFilter(accountKeys: ['0911000001|6']);
      const secondAccount = SyncFilter(accountKeys: ['0911000002|6']);
      final transaction = {
        'bankId': 6,
        'ownerAccountNumber': '0911000002',
        // This may identify a transfer counterparty, not the owning wallet.
        'accountNumber': '0911000001',
      };

      expect(firstAccount.matches(transaction), isFalse);
      expect(secondAccount.matches(transaction), isTrue);
    });

    test('transaction sync fields expose durable owner but not local SIM id',
        () {
      expect(
        SyncEntity.transactions.fieldKeys,
        contains('ownerAccountNumber'),
      );
      expect(
        SyncEntity.transactions.fieldKeys,
        isNot(contains('sourceSubscriptionId')),
      );
      expect(
        SyncEntity.accounts.fieldKeys,
        isNot(contains('smsSubscriptionId')),
      );
    });
  ...
  group('syncEntityRef', () {
    test('transactions use reference', () {
      expect(syncEntityRef(SyncEntity.transactions, {'reference': 'ABC123'}),
          'ABC123');
      expect(
          syncEntityRef(SyncEntity.transactions, {'reference': '  '}), isNull);
    ...
    test('accounts use accountNumber|bank', () {
      expect(
        syncEntityRef(
            SyncEntity.accounts, {'accountNumber': '1234', 'bank': 1}),
        '1234|1',
      );
      expect(syncEntityRef(SyncEntity.accounts, {'accountNumber': '1234'}),
          isNull);
    });
